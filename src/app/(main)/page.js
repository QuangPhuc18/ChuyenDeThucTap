  ...
  const ProductCard = ({ product, isSale, isApi = false }) => {
    ...
    const rawPrice =
      salePriceRaw != null
        ? salePriceRaw
        : isApi
        ? product.price_buy ?? product.price
        : product.price;

    const displayPrice = formatPrice(rawPrice);
    const oldPriceDisplay =
      isSale && originalPriceRaw != null
        ? formatPrice(originalPriceRaw)
        : isApi && product.oldPrice
        ? product.oldPrice
        : product.oldPrice;
    ...
    return (
      <div className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 h-full flex flex-col">
          ...
          {badge && (
            <div
              className={`absolute top-4 left-4 ${
                isSale ? 'bg-gradient-to-r from-red-500 to-pink-500' : 'bg-gradient-to-r from-amber-500 to-orange-500'
              } text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg flex items-center gap-1`}
            >
        ...
        <div className="p-6 flex flex-col flex-grow">
          ...
          <p className="text-gray-600 text-sm mb-4 line-clamp-2 flex-grow">{desc}</p>

          <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
            <div className="flex flex-col">
              {isSale && oldPriceDisplay && <span className="text-sm text-gray-400 line-through mb-1">{oldPriceDisplay} VND</span>}
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  {saleProducts.length > 0 ? (
                    saleProducts.slice(saleIndex, saleIndex + 4).map((product) => (
                      <ProductCard key={product.uniqueKey} product={product} isSale={true} isApi={true} />
                    ))
                  ) : (
                    <p className="col-span-4 text-center text-gray-500">No flash sales currently available.</p>
                  )}
                </div>
                ...
                {saleProducts.length > 4 && (
                  <>
                    ...
                    <button
                      onClick={() => setSaleIndex(Math.min(Math.max(0, saleProducts.length - 4), saleIndex + 1))}
                      disabled={saleIndex >= saleProducts.length - 4}
                      className={`absolute -right-6 top-1/2 -translate-y-1/2 bg-white hover:bg-red-600 text-red-600 hover:text-white rounded-full p-4 transition-all duration-300 shadow-xl hover:shadow-2xl hidden xl:flex items-center justify-center transform hover:scale-110 ${
                        saleIndex >= saleProducts.length - 4 ? 'opacity-50 cursor-not-allowed' : ''
                      }`}
                    >
            ...
            {products.length > 4 && (
              <>
                <button
                  ...
                  className={`absolute -left-6 top-1/2 -translate-y-1/2 bg-white hover:bg-amber-600 text-amber-600 hover:text-white rounded-full p-4 transition-all duration-300 shadow-xl hover:shadow-2xl hidden xl:flex items-center justify-center transform hover:scale-110 ${
                    topSectionIndex === 0 ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                >
                  <ChevronLeft className="w-6 h-6" />
                </button>
                <button
                  ...
                  className={`absolute -right-6 top-1/2 -translate-y-1/2 bg-white hover:bg-amber-600 text-amber-600 hover:text-white rounded-full p-4 transition-all duration-300 shadow-xl hover:shadow-2xl hidden xl:flex items-center justify-center transform hover:scale-110 ${
                    topSectionIndex >= products.length - 4 ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                >
                  <ChevronRight className="w-6 h-6" />
                </button>
            ...
            {products.length > 5 && (
              <>
                <button
                  ...
                  className={`absolute -left-6 top-1/2 -translate-y-1/2 bg-white hover:bg-pink-600 text-pink-600 hover:text-white rounded-full p-4 transition-all duration-300 shadow-xl hover:shadow-2xl hidden xl:flex items-center justify-center transform hover:scale-110 ${
                    bottomSectionIndex === 0 ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                >
                  <ChevronLeft className="w-6 h-6" />
                </button>
                <button
                  ...
                  className={`absolute -right-6 top-1/2 -translate-y-1/2 bg-white hover:bg-pink-600 text-pink-600 hover:text-white rounded-full p-4 transition-all duration-300 shadow-xl hover:shadow-2xl hidden xl:flex items-center justify-center transform hover:scale-110 ${
                    bottomSectionIndex >= products.length - 4 ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                >
                  <ChevronRight className="w-6 h-6" />
                </button>
          ...
          <div className="flex flex-col md:flex-row items-center gap-12">
            <div className="flex-1 relative group">
              ...
              <img
                ...
                alt="Coffee beans"
                className="relative w-full h-80 object-cover rounded-3xl shadow-2xl transform group-hover:scale-105 transition-transform duration-500"
              />
            </div>
            ...
            <div className="flex-1 text-center text-white">
              <h2 className="text-4xl md:text-5xl font-bold mb-6 leading-tight">
                Check Out Our
                <br />
                <span className="text-amber-300">Premium Coffee Beans</span>
              </h2>
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((testimonial) => (
              <div
                key={testimonial.id}
                className="bg-gradient-to-br from-gray-50 to-amber-50/30 rounded-3xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2"
              >
        ...
        <div className="container mx-auto max-w-4xl text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 max-w-xl mx-auto">
            <input
              ...
              className="flex-1 px-6 py-4 rounded-full focus:outline-none focus:ring-4 focus:ring-amber-300"
            />
            <button className="bg-white text-amber-900 px-8 py-4 rounded-full font-bold hover:bg-amber-100 transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:scale-105">
              Subscribe
            </button>
          </div>
        </div>
