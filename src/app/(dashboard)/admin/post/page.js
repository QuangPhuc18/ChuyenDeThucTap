...
import TopicService from '../../../../services/TopicService'; // ✅ Thêm Service Topic

// Đảm bảo URL ảnh đúng chuẩn
...
export default function PostPage() {
  ...
  const [topics, setTopics] = useState([]); // ✅ State lưu danh sách Topic
  ...
  // --- 1. Lấy danh sách Topic khi trang load ---
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const res = await TopicService.getAllActive(); // Gọi hàm lấy Topic active
        if (res && res.status) {
          setTopics(res.data || []);
        }
      } catch (error) {
  ...
  const handleEdit = (post) => {
    ...
    setFormData({
      id: post.id,
      topic_id: post.topic_id || '', // Gán đúng topic_id
      title: post.title || '',
      slug: post.slug || '',
      image: null,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append('topic_id', String(formData.topic_id || '')); // Gửi topic_id
  ...
  // Helper để lấy tên Topic từ ID (cho hiển thị bảng)
  const getTopicName = (id) => {
      const t = topics.find(topic => topic.id === Number(id));
      return t ? t.name : '-';
  };
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex gap-3 items-center">
            ...
            {/* ✅ Filter Topic: Dùng Select Box thay vì Input Text */}
            <select 
              value={filterTopic} 
              onChange={(e) => setFilterTopic(e.target.value)} 
              className="px-3 py-2 border rounded w-48 dark:bg-gray-800 dark:border-gray-600 dark:text-white"
            >
              ...
              {items.map(p => (
                <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-gray-500 max-w-xs truncate">{p.slug}</div>
                  </td>
                  {/* ✅ Hiển thị Tên Topic thay vì ID */}
                  <td className="px-6 py-4 text-gray-600 dark:text-gray-400">
                      {getTopicName(p.topic_id)}
                  </td>
                  <td className="px-6 py-4 text-gray-600 dark:text-gray-400 capitalize">{p.post_type}</td>
                  <td className="px-6 py-4">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* ✅ Select Topic: Hiển thị danh sách Topic động */}
                <div>
                  <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">Topic</label>
                ...
                <div>
                  <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">Post Type</label>
              ...
              <div className="flex justify-end gap-3 mt-8 pt-6 border-t dark:border-gray-700">
                <button type="button" onClick={() => setShowModal(false)} 
                  className="px-6 py-3 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800">Cancel</button>
                <button type="submit" disabled={loading} 
                  className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
                  <Save className="w-5 h-5" /> {loading ? 'Saving...' : (editMode ? 'Update' : 'Create')}
                </button>
              </div>
            </form>
          </div>
