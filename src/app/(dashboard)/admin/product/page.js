...
export default function ProductsPage() {
  ...
  const fetchProducts = useCallback(
    async (page = 1, search = '') => {
      setLoading(true);
      try {
        const data = await ProductService.getList({ page, limit: 10, search });

        const list = normalizeList(data);
        setProducts(list);

        const meta = normalizeMeta(data, page, pagination.per_page);
        // nếu meta.total = 0 nhưng list có dữ liệu, cập nhật total = list.length
  ...
  const handleEditProduct = (product) => {
    ...
    const normalized = (Array.isArray(listImgs) ? listImgs : [])
      .map((img) => {
        ...
        if (img && img.image)
          return { id: img.id || null, url: img.image.startsWith('http') ? img.image : IMAGE_BASE_URL + img.image };
        return null;
      })
      .filter(Boolean);
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        Number(product.status) === 1 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}
                    >
                      {Number(product.status) === 1 ? 'Active' : 'Inactive'}
                    </span>
