...
export default function ProductsPage() {
  ...
  const attributes = [{ id: 1, name: 'Size' }];
  ...
  const normalizeMeta = (payload, page, perPage) => {
  ...
  // ========================================================================
  // 1. FETCH DATA (QUAN TRỌNG NHẤT: ADMIN VIEW)
  // ========================================================================
    ...
    async (page = 1, search = '') => {
      ...
      try {
        const data = await ProductService.getList({ 
            ...
            limit: 10, 
            search,
            // [CỜ QUAN TRỌNG]: Giúp Admin thấy cả sản phẩm ẩn (Status 0)
            // Nếu thiếu cờ này, sản phẩm vừa tạo sẽ không hiện ra.
            admin_view: true 
        });
        ...
        const meta = normalizeMeta(data, page, pagination.per_page);
        ...
        setProducts([]);
        setPagination((prev) => ({ ...prev, total: 0, current_page: 1, last_page: 1 }));
      } finally {
        setLoading(false);
      }
    },
  ...
  const addAttributeGroup = () => {
    setAttributeGroups((prev) => [...prev, { id: Date.now().toString(), attribute_id: '', values: [], input: '' }]);
  };
  const removeAttributeGroup = (groupId) => setAttributeGroups((prev) => prev.filter((g) => g.id !== groupId));
    ...
    if (Array.isArray(pa) && pa.length > 0) {
      if (typeof pa[0] === 'object' && pa[0] !== null && 'value' in pa[0]) {
        ...
        return Object.keys(map).map((aid) => ({
          attribute_id: aid,
          values: Array.from(new Set(map[aid])),
        }));
      }
  ...
  const clearGallerySelections = () => {
    galleryPreviews.forEach((url) => { try { URL.revokeObjectURL(url); } catch {} });
    setGalleryPreviews([]);
    setGalleryFiles([]);
    setExistingGallery([]);
  };
  ...
  const handleNameChange = (e) => {
    ...
    const slug = String(name || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/đ/g, 'd').replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-');
  ...
  const handleAddProduct = () => {
    ...
    setFormData({
      ...
      content: '', description: '', price_buy: '', status: 1, // Trên form để 1, nhưng backend sẽ ép về 0
    });
    setAttributeGroups([]);
  ...
  const handleEditProduct = (product) => {
    ...
    const normalized = (Array.isArray(listImgs) ? listImgs : [])
      .map((img) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Nếu là Edit: Gửi status hiện tại.
      // Nếu là Create: Backend sẽ tự động set status = 0 (để chờ nhập kho)
      if(editMode) {
      ...
      if (formData.thumbnail && formData.thumbnail instanceof File) data.append('thumbnail', formData.thumbnail);
      if (galleryFiles.length > 0) galleryFiles.forEach((file) => data.append('images[]', file));
      ...
      if (response.status) {
        alert(editMode ? 'Cập nhật thành công!' : 'Thêm mới thành công! (Sản phẩm đang ẩn, hãy nhập kho)');
        setShowModal(false);
        setAttributeGroups([]);
        clearGallerySelections();
        // Gọi lại fetchProducts để cập nhật danh sách (bao gồm cả SP ẩn vừa tạo)
        fetchProducts(pagination.current_page, searchTerm);
      } else {
              ...
              {safeProducts.map((product) => (
                <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="flex gap-1 mt-1">
                              {display.map((a) => (
                                <span key={a} className="text-xs px-2 py-0.5 bg-gray-100 rounded">
                                  {a}
                                </span>
                              ))}
                            </div>
                  ...
                  <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                    {formatPrice(product.price_buy || product.price)}
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        Number(product.status) === 1 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                      }`}
                    >
                      {Number(product.status) === 1 ? 'Active' : 'Hidden'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {galleryPreviews.length > 0 &&
                      galleryPreviews.map((url, idx) => (
                        <div key={`new-${idx}`} className="w-20 h-20 border rounded-lg overflow-hidden relative">
                          ...
                          <button type="button" onClick={() => removeNewGalleryImage(idx)} className="absolute right-1 top-1 bg-white/80 rounded px-1 text-sm">
                            ×
                          </button>
                        </div>
                      ))}
                ...
                <div className="md:col-span-2">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button type="button" onClick={addAttributeGroup} className="text-sm px-3 py-1 bg-green-500 text-white rounded">
                      Thêm thuộc tính
                    </button>
                  </div>
                      ...
                      <div key={group.id} className="p-3 border rounded-lg bg-gray-50">
                        <div className="flex items-center gap-3 mb-2">
                          <select
                            ...
                            onChange={(e) => setGroupField(group.id, 'attribute_id', e.target.value)}
                            ...
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter' || e.key === ',') {
                                  e.preventDefault();
                                  addValuesToGroup(group.id);
                                }
                              }}
                              onChange={(e) => setGroupField(group.id, 'input', e.target.value)}
                              placeholder="Nhập giá trị (nhấn Enter hoặc dấu phẩy để thêm)"
                              className="w-full px-3 py-2 border rounded"
                            />
                          </div>

                          <button type="button" onClick={() => addValuesToGroup(group.id)} className="px-3 py-2 bg-blue-600 text-white rounded">
                            Thêm
                          </button>

                          <button type="button" onClick={() => removeAttributeGroup(group.id)} className="ml-2 text-red-600">
                            Xóa
                          </button>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {group.values.map((v) => (
                            <div key={v} className="flex items-center gap-2 bg-white border px-3 py-1 rounded">
                              ...
                              <button type="button" onClick={() => removeValueFromGroup(group.id, v)} className="text-red-500">
                                ×
                              </button>
                            </div>
                          ))}
